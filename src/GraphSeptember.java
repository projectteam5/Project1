...
public class GraphSeptember extends JPanel {
   ...
   public GraphSeptember(String applicationTitle, String chartTitle) {
		...
		for(ToHoldSoldProductsAndQuantity g: completeList){
			if(g.getDate().contains("Sep")){sepList.add(g);}}	
...
// Function to check for two or more products sold in the same month that
   ...
   public void resortMonthlyListToCalculateMulipleSalesOfProducts(){	
		// January
		for(ToHoldSoldProductsAndQuantity j: sepList){storeAndCheckForMoreSalesOfProduct(j);}
