...
public class GraphAugust extends JPanel {
   ...
   public GraphAugust(String applicationTitle, String chartTitle) {
		...
		for(ToHoldSoldProductsAndQuantity g: completeList){
			if(g.getDate().contains("Aug")){augList.add(g);}}	
...
// Function to check for two or more products sold in the same month that
   ...
   public void resortMonthlyListToCalculateMulipleSalesOfProducts(){	
		// January
		for(ToHoldSoldProductsAndQuantity j: augList){storeAndCheckForMoreSalesOfProduct(j);}
