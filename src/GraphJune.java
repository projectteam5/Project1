...
public class GraphJune extends JPanel {
   ...
   public GraphJune(String applicationTitle, String chartTitle) {
		...
		for(ToHoldSoldProductsAndQuantity g: completeList){
			if(g.getDate().contains("Jun")){junList.add(g);}}	
...
// Function to check for two or more products sold in the same month that
   ...
   public void resortMonthlyListToCalculateMulipleSalesOfProducts(){	
		// January
		for(ToHoldSoldProductsAndQuantity j: junList){storeAndCheckForMoreSalesOfProduct(j);}
