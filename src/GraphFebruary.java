...
public class GraphFebruary extends JPanel {
   ...
   public GraphFebruary(String applicationTitle, String chartTitle) {
		...
		for(ToHoldSoldProductsAndQuantity g: completeList){
			if(g.getDate().contains("Feb")){febList.add(g);}}	
...
// Function to check for two or more products sold in the same month that
   ...
   public void resortMonthlyListToCalculateMulipleSalesOfProducts(){	
		// January
		for(ToHoldSoldProductsAndQuantity j: febList){storeAndCheckForMoreSalesOfProduct(j);}
