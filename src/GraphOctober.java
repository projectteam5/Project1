...
public class GraphOctober extends JPanel {
   ...
   public GraphOctober(String applicationTitle, String chartTitle) {
		...
		for(ToHoldSoldProductsAndQuantity g: completeList){
			if(g.getDate().contains("Oct")){octList.add(g);}}	
...
// Function to check for two or more products sold in the same month that
   ...
   public void resortMonthlyListToCalculateMulipleSalesOfProducts(){	
		// January
		for(ToHoldSoldProductsAndQuantity j: octList){storeAndCheckForMoreSalesOfProduct(j);}
